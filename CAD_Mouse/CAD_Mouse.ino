...
const int   activationRadius         = 40;    // How far from the center does the switch need to be to actually activate MMB.
...
#define     DOWN                       0
#define     UP                         1
bool        joystick_pressed         = false; // Is true when the joystick switch is pressed and false when the joystick switch is released.
...
const int   rolling_step_multiplier   = 15;    // Governs mouse sensitivity for rolling. The higher, the faster the roll.
const int   zoom_pulse_threshold      = 4;     // Governs the net number of pulses in the same direction required before initating mouse movement.
int         zoom_pulse_counter        = 0;
...
bool        rotary_pressed            = false; // Is true when the rotary switch is pressed and false when the rotary switch is released.
...
bool outside_activationRadius(int stick_x, int stick_y) {
  ...
  #ifdef SerialDebugger
  Serial.println(sqrt(square(center_x) + square(center_y)));
  #endif
...
void loop() {
  ...
  int joy_switch_signal = digitalRead(joy_switch);
  if (joy_switch_signal == DOWN and !joystick_pressed) {
    #ifdef SerialDebugger
  ...
  } else if (joy_switch_signal == UP and joystick_pressed) {
    #ifdef SerialDebugger
  ...
  if (rotary_switch_signal == DOWN and !rotary_pressed) {
    #ifdef SerialDebugger
  ...
  } else if (rotary_switch_signal == UP and rotary_pressed) {
    #ifdef SerialDebugger
  ...
  if (!MMB_pressed && outside_activationRadius(stick_x, stick_y)) {
    if (joystick_mode == PANNING) {
      ...
      #ifdef SerialDebugger
      Serial.println("JOYSTICK ACTIVATED KEYBOARD ONLINE");
      #endif
      Keyboard.press(KEY_LEFT_CTRL);
    }
    Mouse.begin();
    #ifdef SerialDebugger
    Serial.println("JOYSTICK ACTIVATED MOUSE ONLINE");
    #endif
    mouse_isAlive = true;
    if (!Mouse.isPressed(MOUSE_MIDDLE)) {
      Mouse.press(MOUSE_MIDDLE);
      #ifdef SerialDebugger
      Serial.println("JOYSTICK ACTIVATED MIDDLE CLICK");
      #endif
    }
  ...
  if (MMB_pressed && outside_activationRadius(stick_x, stick_y)) {
    ...
    if (!Mouse.isPressed(MOUSE_MIDDLE)) {
      Mouse.press(MOUSE_MIDDLE);
      #ifdef SerialDebugger
      Serial.println("RETURN MOUSE MIDDLE MOUSE CLICK");
      #endif
    }
  } else 
  ...
  if (MMB_pressed && !outside_activationRadius(stick_x, stick_y)) {
    if (joystick_mode == PANNING) {
      ...
      Keyboard.end();
      #ifdef SerialDebugger
      Serial.println("JOYSTICK DEACTIVATED KEYBOARD OFFLINE");
      #endif
    ...
    Mouse.end();
    #ifdef SerialDebugger
    Serial.println("JOYSTICK DEACTIVATED MOUSE OFFLINE");
    #endif
    mouse_isAlive = false;
    MMB_pressed = false;
    ...
    if (rotary_signal == 0 and remaining_roll == 0) {
      ...
      if (rotary_mode == ROLLING) {
        // Roll mode
        // Add to existing remaining roll if there is input from the rotary. Otherwise, execute mouse movement to eliminate remaining roll.
        remaining_roll += rolling_step_multiplier * rotary_signal;
        rotary_move_mouse();
      } else {
        // Zoom mode
        // Count pulses from rotary. If pulses exceed threshold, scroll the mouse in the appropriate direction.
        zoom_pulse_counter += rotary_signal;
        if (abs(zoom_pulse_counter) >= zoom_pulse_threshold) {
          Mouse.move(0, 0, rotary_signal);
        }
      }
    }
  }
